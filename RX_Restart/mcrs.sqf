...
private["_restartTimeInterval","_restartTime","_sleepInterval","_msgTimeInterval", "_useMSG", "_password", "_useShutdown", "_startTime", "_timeUntilRestart", "_timeSinceStart", "_timeUntilRestartMin"];
...
_password = call compile preprocessFileLineNumbers "\userConfig\myPass\myPass.hpp";
...
_timeUntilRestartMin = [_timeUntilRestart,"HH:MM:SS",false] call BIS_fnc_secondsToString; 
...
{
  {
    if((_timeUntilRestart <= _x) AND (_timeUntilRestart > _msgTimeInterval select (_forEachIndex +1))) then
    {
      ...
      {
        //_message = format [_msg, _timeUntilRestart]; //decimal seconds
		//_message = ["Restart",[format[_msg, round (_timeUntilRestart)]]]; // seconds
		_message = ["Restart",[format[_msg, _timeUntilRestartMin]]]; //minutes
        _message remoteExec ['bis_fnc_showNotification',0];
